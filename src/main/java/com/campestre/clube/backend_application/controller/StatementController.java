...
public class StatementController {
    ...
    public ResponseEntity<StatementResponseDto> register(@RequestBody @Valid StatementRequestDto dto){
        return ResponseEntity.status(HttpStatus.CREATED).body(StatementResponseDto.toResponse(
    ...
    public ResponseEntity<List<StatementResponseDto>> getAll(){
        ...
        return ResponseEntity.status(HttpStatus.OK).body(statements.stream().map(StatementResponseDto::toResponse)
                .toList());
    }

    @GetMapping("/{id}")
    public ResponseEntity<StatementResponseDto> getById(@PathVariable Integer id){
        return ResponseEntity.status(HttpStatus.OK).body(StatementResponseDto.toResponse(statementService.getById(id)));
    }
